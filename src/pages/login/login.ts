...
import { NavController, NavParams, Platform, Events, App, Nav } from 'ionic-angular';
      ...
      browserRef.on("loadstart").subscribe((event) => {
        if ((event.url).indexOf("http://localhost/callback") === 0) {
          ...
          if (responseParameters[0].indexOf('error') == -1) {
            ...
            reject({ detail: 'Lo sentimos ocurrió un problema, inténtalo de nuevo.', state: 'error_noti' });
          }
        }
      });
      browserRef.on("exit").subscribe((event) => {
        if (!closeSuccess) {
          reject({ detail: 'El usuario canceló la autorización.', state: 'error' });
        }
      });
    });
  }
